...
这是一个专为解决 Word 文档格式混乱问题而设计的极简工具。它采用了现代化的纸质感 UI 设计，能够自动识别文档中的标点、排版和字体问题，并可以根据国家标准（GB/T 9704-2012）进行一键自动化修复。
...
* **🛠️ 小问题修复** — 修复了一些小 Bug 和小问题，优化使用体验

**当前开发分支新增：**

* **📐 通用文档排版** — 可自定义段落类型和本地匹配规则，支持章、条、语录等非公文结构
* **📄 从样例学习样式** — 仅在本机读取 `.docx` 的页面、字体、缩进和行距，不复制正文内容
* **1️⃣ 自动编号规范化** — 将可解析的 Word 自动编号固化为可见文字，避免标题识别和重复编号问题
* **📊 表格与页面增强** — 支持表格引导段保护、单元格四边边距、文字自动适应、纸张尺寸和横竖方向
* **📋 文本转 Word** — 可粘贴纯文本或 Markdown 生成 `.docx`
* **🖥️ 界面适配** — 更新现代控件、高 DPI 缩放和窗口自适应布局

**v1.8.8 更新：**

* **📄 页码设置优化** — 支持四种页码样式、居中或奇偶页外侧位置，并可安全替换已有页码而不覆盖正常页脚内容
* **🈶 中文换行规则** — 为正文、表格和页眉页脚启用中文禁则，避免标点出现在新行行首
* **🔤 标题字体细分** — 一级至四级标题均可分别设置中文字体与英文/数字字体
* **🖼️ 图片与附件保护** — 保留内嵌图片和媒体节点；独立“附件/附件1”等标记另起页并保持后续标题格式

## 使用许可

您当前使用的是免费开源版（GitHub）。从 `v1.8.8.3` 起，本版本依据 [PolyForm Noncommercial License 1.0.0](LICENSE) 提供，仅限个人和非商业用途免费使用；未经许可，不得销售、收费分发或用于其他商业目的。
...
3. **🔤 字体自动适配**：自动识别小标题与正文层级，匹配对应的字体（如黑体、仿宋）和字号。
...
10. **📊 表格自动调整**：识别文档中的表格，自动调整列宽、行高及单元格格式。默认保留原始对齐方式，自定义模式下可开启按内容类型对齐（数字靠右、短文本居中等）。
...
14. **🧩 通用段落规则**：自定义段落类型、字体、缩进、行距和本地正则匹配规则，规则在本机确定性执行。
15. **📄 样例样式学习**：从排版正确的 `.docx` 中提取可复用样式参数，并在保存前预览确认。
16. **📐 页面与单元格增强**：自定义纸张尺寸和横竖方向，并细化单元格边距和文字适应选项。
...
| 模式 | 适用场景 |
|------|----------|
| **🪄 一键处理** | **(推荐)** 全自动模式。同时进行标点修复、排版规范和样式清洗，一步到位。 |
...
🔒 **本工具所有操作均在本地完成**，不会上传任何文档内容到服务器或云端。无网络通信、无数据收集、无遥测追踪。源代码公开，欢迎审查。
...
从 `v1.8.8.3` 起，本项目采用 [PolyForm Noncommercial License 1.0.0](LICENSE)：个人和非商业用途可以免费使用、修改和按许可证要求分发；其他商业用途需要另行取得授权。
